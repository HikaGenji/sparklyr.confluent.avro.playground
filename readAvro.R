...
# config$'sparklyr.shell.conf' <- paste0("spark.jars.ivySettings=",  "/tmp/ivysettings.xml")

sc <- spark_connect(master = "local", spark_home = "spark", config=config)

stream_read_kafka_avro(sc, "parameter", startingOffsets="earliest", kafkaUrl="broker:9092", schemaRegistryUrl="http://schema-registry:8081") %>%
...
stream_write_kafka_avro(sc, topic="output", dataFrame=., kafkaUrl="broker:9092", schemaRegistryUrl="http://schema-registry:8081")

# read avro data and push to memory
stream_read_kafka_avro(sc, "output", startingOffsets="earliest", kafkaUrl="broker:9092", schemaRegistryUrl="http://schema-registry:8081") %>%
invoke("select", "value.timestamp", list("value.id", "value.side")) %>%
sdf_register() %>%
mutate(side=2*side) %>%
group_by(id) %>%
